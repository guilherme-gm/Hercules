...
#ifdef RENEWAL_EDP
	// @FIXME: This seems a separate fix, about meteor assault not being affected by EDP. And soul breaker should.
	//         Needs confirmation...
...
static int64 battle_calc_masteryfix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int64 damage, int div, bool left, bool weapon)
{
	...
	if( sc ){ // sc considered as masteries
		...
		if (sc->data[SC_PLATINUM_ALTER])
			damage += sc->data[SC_PLATINUM_ALTER]->val2;
#if 0
