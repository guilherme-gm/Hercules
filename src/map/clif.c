...
static void clif_zc_quest_dialog(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	const size_t len = msgLen + sizeof(struct PACKET_ZC_QUEST_DIALOG);
...
static void clif_zc_monolog_dialog(struct map_session_data *sd, int npcid, const char *mes)
{
#if PACKETVER_ZERO_NUM >= 20210721
	nullpo_retv(sd);
	nullpo_retv(mes);
	...
	const size_t msgLen = strlen(mes);
	const size_t len = msgLen + sizeof(struct PACKET_ZC_MONOLOG_DIALOG);
	Assert_retv(len <= INT16_MAX);
...
/// Displays an NPC dialog menu (ZC_QUEST_DIALOG_MENU_LIST).
static void clif_zc_quest_dialog_menu_list(struct map_session_data *sd, int npcid, const char *mes)
{
#if PACKETVER_ZERO_NUM >= 20210721
	nullpo_retv(sd);
	nullpo_retv(mes);
	...
	const size_t msgLen = strlen(mes);
	const size_t len = msgLen + sizeof(struct PACKET_ZC_QUEST_DIALOG_MENU_LIST);
	Assert_retv(len <= INT16_MAX);
...
static void clif_inventoryExpandAck(struct map_session_data *sd, enum expand_inventory result, int itemId)
{
	...
	p->result = result;
	p->itemId = itemId;
	WFIFOSET(fd, sizeof(struct PACKET_ZC_ACK_OPEN_MSGBOX_EXTEND_BODYITEM_SIZE));
#endif
}
...
static void clif_party_inviteack(struct map_session_data *sd, const char *nick, int result)
{
	...
	if (result == 7) {
		clif->message(fd, msg_sd(sd, MSGTBL_CHARACTER_NOT_FOUND)); // Character not found.
		return;
	}
#endif
...
static void clif_sendegg(struct map_session_data *sd)
{
	...
	if (map->list[sd->bl.m].flag.nopet != 0) {
		clif->message(fd, msg_sd(sd, MSGTBL_PETS_DISABLED_IN_MAP)); // "Pets are disabled in this map."
		return;
	}
...
static void clif_guild_basicinfo(struct map_session_data *sd)
{
	...
	p->emblemVersion = g->emblem_id;
	memcpy(p->guildname, g->name, NAME_LENGTH);
	safestrncpy(p->manageLand, msg_sd(sd, MSGTBL_CASTLES_0 + guild->checkcastles(g)), MAP_NAME_LENGTH_EXT);
	p->zeny = 0;
...
static void clif_guild_castleinfo(struct map_session_data *sd, struct guild_castle *gc)
{
	...
	struct PACKET_ZC_REQ_ACK_AGIT_INVESTMENT p = { 0 };
...
static void clif_homname_ack(int fd, struct block_list *bl)
{
	...
	struct PACKET_ZC_ACK_REQNAMEALL_NPC packet = { 0 };
	...
	clif->send_selforarea(fd, bl, &packet, sizeof(struct PACKET_ZC_ACK_REQNAMEALL_NPC));
}
...
static void clif_parse_LoadEndAck(int fd, struct map_session_data *sd)
{
	...
	if (sd->pd != NULL) {
		if (map->list[sd->bl.m].flag.nopet != 0) { // Return the pet to egg. [Skotlex]
			clif->message(sd->fd, msg_sd(sd, MSGTBL_PETS_DISABLED_IN_MAP)); // Pets are not allowed in Guild Wars.
...
#else
		clif->messagecolor_self(fd, COLOR_WHITE, msg_fd(fd, MSGTBL_WORK_IN_PROGRESS_TRY_AGAIN));
#endif
		return;
	}
...
static void clif_noask_sub(struct map_session_data *src, struct map_session_data *target, int type)
{
	...
	msg = msg_sd(src, MSGTBL_NOASK_REJECTED);
	clif_disp_onlyself(src, msg);
	//Notice that a request was rejected.
	snprintf(output, 256, msg_sd(target,393+type), src->status.name, MSGTBL_RELOAD_STATUS_RELOADED);
...
static void clif_parse_GuildLeave(int fd, struct map_session_data *sd)
{
	...
	if (sd->bg_id) {
		clif->message(fd, msg_fd(fd, MSGTBL_CANNOT_LEAVE_BG_GUILD)); //"You can't leave battleground guilds."
		return;
	}
...
static void clif_parse_GM_Monster_Item(int fd, struct map_session_data *sd)
{
	...
	if (strcmp("onekillmonster", item_monster_name) == 0 && pc->can_use_command(sd, "@killmonster")) {
		...
		if (sd->state.onekillmonster != 0)
			clif->message(fd, msg_fd(fd, MSGTBL_MONSTER_KILL_STARTED)); // Instant monster kill state started.
		else
			clif->message(fd, msg_fd(fd, MSGTBL_MONSTER_KILL_ENDED)); // Instant monster kill state ended.
		return;
	}
}
...
static void clif_parse_FriendsListAdd(int fd, struct map_session_data *sd)
{
	...
	for (i = 0; i < MAX_FRIENDS && sd->status.friends[i].char_id != 0; i++) {
		if (sd->status.friends[i].char_id == f_sd->status.char_id) {
			clif->message(fd, msg_fd(fd, MSGTBL_FRIEND_ALREADY_EXISTS)); //"Friend already exists."
			return;
		}
	}
...
static void clif_parse_FriendsListRemove(int fd, struct map_session_data *sd)
{
	...
	if (i == MAX_FRIENDS) {
		clif->message(fd, msg_fd(fd, MSGTBL_NAME_NOT_FOUND)); //"Name not found in list."
		return;
	}
	...
	clif->message(fd, msg_fd(fd, MSGTBL_FRIEND_REMOVED)); //"Friend removed"
...
static void clif_parse_cz_config(int fd, struct map_session_data *sd)
{
	...
	case CZ_CONFIG_PET_AUTOFEEDING: {
		...
		if (pd->petDB->autofeed == 0) {
			clif->message(fd, msg_sd(sd, MSGTBL_AUTOFEED_DISABLED)); // "Autofeed is disabled for this pet."
			return;
		}
...
{ // '<ItemName>' item will disappear in <seconds/60> minutes.
	...
	p->itemId = nameid;
	p->seconds = seconds;
	WFIFOSET(fd, sizeof(struct PACKET_ZC_CASH_TIME_COUNTER));
}
...
{ // '<ItemName>' item has been deleted from the Inventory
	...
	p->index = index + 2;
	p->itemId = nameid;
	WFIFOSET(fd, sizeof(struct PACKET_ZC_CASH_ITEM_DELETE));
}
...
#endif
	WFIFOSET(fd, sizeof(struct PACKET_ZC_UPDATE_ITEM_FROM_BUYING_STORE));
}
...
static bool clif_show_monster_hp_bar(struct block_list *bl)
{
	...
	if (status_get_hp(bl) < status_get_max_hp(bl)) {
		if ((battle->bc->show_monster_hp_bar & 1) != 0 && md->class_ != MOBID_EMPELIUM && (md->status.mode & MD_BOSS) == 0 && md->guardian_data == NULL)
			return true;
		if ((battle->bc->show_monster_hp_bar & 2) != 0 && (md->class_ == MOBID_EMPELIUM || md->guardian_data != NULL))
			return true;
		if ((battle->bc->show_monster_hp_bar & 4) != 0 && (md->status.mode & MD_BOSS) != 0 && md->class_ != MOBID_EMPELIUM && md->guardian_data == NULL)
			return true;
	}
	return false;
}
...
static void clif_parse_BankDeposit(int fd, struct map_session_data *sd)
{
	...
	if (!battle_config.feature_banking) {
		clif->messagecolor_self(fd, COLOR_RED, msg_fd(fd, MSGTBL_BANKING_DISABLED));
		return;
	}
...
static void clif_show_modifiers(struct map_session_data *sd)
{
	...
	if (sd->status.mod_exp != 100 || sd->status.mod_drop != 100 || sd->status.mod_death != 100) {
		...
		snprintf(output, sizeof(output), msg_sd(sd, MSGTBL_BASE_EXP_DROP_PENALTY), sd->status.mod_exp, sd->status.mod_drop, sd->status.mod_death);
...
static void clif_parse_RouletteOpen(int fd, struct map_session_data *sd)
{
	...
	if( !battle_config.feature_roulette ) {
		clif->message(fd, msg_fd(fd, MSGTBL_ROULETTE_DISABLED)); // Roulette is disabled
		return;
	}
...
static void clif_change_title_ack(int fd, struct map_session_data *sd, int title_id)
{
	...
	if (!achievement->check_title(sd, title_id)) {
		clif->message(fd, msg_sd(sd, MSGTBL_TITLE_NOT_EARNED)); // "Title is not yet earned."
		failed = 1;
	}
...
/// Send hat effects to the client (ZC_EQUIPMENT_EFFECT).
...
static void clif_parse_attendance_reward_request(int fd, struct map_session_data *sd)
{
	...
	sprintf(title, msg_txt(MSGTBL_ATTENDANCE_MAIL_TITLE), attendance_count + 1);
	sprintf(body, msg_txt(MSGTBL_ATTENDANCE_MAIL_TITLE), attendance_count + 1);
	...
	safestrncpy(msg.sender_name, msg_txt(MSGTBL_ATTENDANCE_MAIL_SENDER), NAME_LENGTH);
...
static void clif_camera_showWindow(struct map_session_data *sd)
{
	...
	nullpo_retv(sd);
	struct PACKET_ZC_VIEW_CAMERAINFO p;
	p.packetType = HEADER_ZC_VIEW_CAMERAINFO;
	p.action = 1;
	p.range = 0;
	p.rotation = 0;
	p.latitude = 0;
...
static bool clif_enchant_equipment(struct map_session_data *sd, enum equip_pos pos, int cardSlot, int cardId, int8 equipFlag)
{
	...
	struct PACKET_ZC_UPDATE_CARDSLOT p;
	p.packetType = HEADER_ZC_UPDATE_CARDSLOT;
	p.wearState = pos;
	p.cardSlot = cardSlot;
	p.itemId = cardId;
...
static void clif_npc_barter_open(struct map_session_data *sd, struct npc_data *nd)
{
	...
	int c = 0;
	int maxCount = (sizeof(packet_buf) - sizeof(struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO)) / sizeof(struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO_sub);
	struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO *packet = (struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO*)&packet_buf[0];
	packet->packetType = HEADER_ZC_NPC_BARTER_MARKET_ITEMINFO;
	...
	packet->packetLength = sizeof(struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO) + sizeof(struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO_sub) * c;
	clif->send(packet, packet->packetLength, &sd->bl, SELF);
#endif
}
...
static void clif_npc_expanded_barter_open(struct map_session_data *sd, struct npc_data *nd)
{
	...
	// Here should be sizeof(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub)
	const int ptr_size = sizeof(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub) -
		sizeof(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2);
	for (int i = 0; i < shop_size && buf_left >= ptr_size; i++) {
		if (shop[i].nameid) {
			...
			for (int j = 0; j < count; j ++) {
				struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2 *packet_currency = &item->currencies[j];
	...
	packet->packetLength = sizeof(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO) +
		ptr_size * items_count +
		sizeof(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2) * currencies_count;
	clif->send(packet, packet->packetLength, &sd->bl, SELF);
#endif
}
...
static void clif_AddItemRefineryUIAck(struct map_session_data *sd, int item_index, struct s_refine_requirement *req)
{
	...
	char buf[sizeof(struct PACKET_ZC_REFINING_MATERIAL_LIST) + sizeof(struct PACKET_ZC_REFINING_MATERIAL_LIST_SUB) * MAX_REFINE_REQUIREMENTS];
	struct PACKET_ZC_REFINING_MATERIAL_LIST *p = (struct PACKET_ZC_REFINING_MATERIAL_LIST *)buf;
...
static void clif_announce_refine_status(struct map_session_data *sd, int item_id, int refine_level, bool success, enum send_target target)
{
	...
	struct PACKET_ZC_BROADCAST_ITEMREFINING_RESULT p;
...
static void clif_guild_castleteleport_res(struct map_session_data *sd, enum siege_teleport_result result)
{
	...
	struct PACKET_ZC_REQ_ACK_MOVE_GUILD_AGIT p = { 0 };
...
static void clif_captcha_upload_request(struct map_session_data *sd, const char *captcha_key, const int captcha_flag)
{
	...
	WFIFOSET(fd, sizeof(struct PACKET_ZC_ACK_UPLOAD_MACRO_DETECTOR));
#endif
}
...
static void clif_captcha_preview_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size, const int captcha_flag)
{
	...
	safestrncpy(p->captchaKey, captcha_key, sizeof(p->captchaKey));
	WFIFOSET(fd, sizeof(struct PACKET_ZC_ACK_PREVIEW_MACRO_DETECTOR));
#endif
}
...
static void clif_macro_detector_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size)
{
	...
	p->imageSize = image_size;
	safestrncpy(p->captchaKey, captcha_key, sizeof(p->captchaKey));
	WFIFOSET(fd, sizeof(struct PACKET_ZC_APPLY_MACRO_DETECTOR));
#endif
}
...
static void clif_macro_detector_request_show(struct map_session_data *sd)
{
	...
	WFIFOSET(fd, sizeof(struct PACKET_ZC_REQ_ANSWER_MACRO_DETECTOR));
#endif
}
...
static void clif_parse_macro_reporter_ack(int fd, struct map_session_data *sd)
{
	...
	if (tsd == NULL) {
		char buf[256];
		sprintf(buf, msg_fd(fd, MSGTBL_MACRO_PLAYER_UNAVAILABLE), p->AID); // Player %d is unavailable.
		clif->message(fd, buf);
		return;
	}
...
static void clif_grade_enchant_add_item_result_fail(struct map_session_data *sd)
{
	...
	p->PacketLength = sizeof(struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST);
	p->index = -1;
	WFIFOSET(fd, sizeof(struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST));
#endif
}
...
static void clif_grade_enchant_result(struct map_session_data *sd, int16 index, enum grade_level gl, enum grade_ui_result result)
{
	...
	p->grade = gl;
	p->result = result;
	WFIFOSET(fd, sizeof(struct PACKET_ZC_GRADE_ENCHANT_ACK));
#endif
}
